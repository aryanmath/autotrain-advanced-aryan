...
class AutomaticSpeechRecognitionDataset:
    """
    Minimal, modular dataset class for ASR tasks.
    Args:
        data (Dataset): Dataset containing audio and text data.
        processor (Any): Audio processor for feature extraction.
        config (Any): Training configuration.
        model_type (str): 'seq2seq', 'ctc', or 'generic'.
...
# Utility function for audio file validation (to be called before training, not in __init__)
def validate_audio_files(data, audio_column, sampling_rate, max_duration):
    """
    Validate that all audio files exist and can be loaded, and are within max_duration.
    Raises ValueError if any file is invalid.
    """
    invalid_files = []
    for idx, item in enumerate(data):
        ...
        try:
            ...
            if duration > max_duration:
                invalid_files.append((idx, audio_path, f"Duration {duration:.2f}s exceeds max_duration {max_duration}s"))
        except Exception as e:
