...
class AutomaticSpeechRecognitionParams(AutoTrainParams):
    ...
    def validate_params(self):
        logger.info("[PARAMS] Starting parameter validation...")
        self._validate_model()
        self._validate_data_path()
        if self.using_hub_dataset:
            self._validate_hub_dataset()
        else:
            self._validate_local_dataset()
        self._validate_training_params()
        self._validate_misc()
        logger.info("[PARAMS] Parameter validation complete.")

    def _validate_model(self):
        logger.info(f"[PARAMS] Validating model: {self.model}")
        ...
        for category, models in available_models.items():
            if self.model in models:
                ...
                break
        if not model_found:
            logger.error(f"[PARAMS] Model {self.model} not found in available models: {available_models}")
            raise ValueError(f"Model {self.model} not found in available models: {available_models}")
        logger.info(f"[PARAMS] Model {self.model} is valid.")

    def _validate_data_path(self):
        logger.info(f"[PARAMS] Validating data path: {self.data_path}")
        if not self.data_path:
            logger.error("[PARAMS] data_path must be provided")
            raise ValueError("data_path must be provided")
        logger.info(f"[PARAMS] Data path {self.data_path} is set.")

    def _validate_hub_dataset(self):
        logger.info("[PARAMS] Validating hub dataset parameters...")
        if not self.train_split:
            logger.error("[PARAMS] train_split must be provided when using hub dataset")
            raise ValueError("train_split must be provided when using hub dataset")
        if not self.token:
            logger.error("[PARAMS] token must be provided when using hub dataset")
            raise ValueError("token must be provided when using hub dataset")
        logger.info("[PARAMS] Hub dataset parameters are valid.")

    def _validate_local_dataset(self):
        logger.info("[PARAMS] Validating local dataset parameters...")
        if not os.path.exists(self.data_path):
            logger.error(f"[PARAMS] data_path does not exist: {self.data_path}")
            raise ValueError(f"data_path does not exist: {self.data_path}")
        files = os.listdir(self.data_path)
        csv_files = [f for f in files if f.endswith('.csv')]
        if not csv_files:
            logger.error(f"[PARAMS] No CSV file found in {self.data_path}")
            raise ValueError(f"No CSV file found in {self.data_path}")
        audio_folder = os.path.join(self.data_path, 'audio')
        if not os.path.exists(audio_folder):
            logger.warning(f"[PARAMS] Audio folder not found: {audio_folder}")
            logger.warning("[PARAMS] Will try to use audio paths as provided in CSV")
        logger.info("[PARAMS] Local dataset parameters are valid.")

    def _validate_training_params(self):
        logger.info("[PARAMS] Validating training parameters...")
        if self.batch_size <= 0:
            logger.error("[PARAMS] batch_size must be positive")
            raise ValueError("batch_size must be positive")
        if self.gradient_accumulation <= 0:
            logger.error("[PARAMS] gradient_accumulation must be positive")
            raise ValueError("gradient_accumulation must be positive")
        if self.epochs <= 0:
            logger.error("[PARAMS] epochs must be positive")
            raise ValueError("epochs must be positive")
        if self.lr <= 0:
            logger.error("[PARAMS] learning rate must be positive")
            raise ValueError("learning rate must be positive")
        logger.info("[PARAMS] Training parameters are valid.")

    def _validate_misc(self):
        logger.info("[PARAMS] Validating miscellaneous parameters...")
        if self.mixed_precision not in ["no", "fp16", "bf16"]:
            logger.error("[PARAMS] mixed_precision must be one of: no, fp16, bf16")
        ...
        if self.scheduler not in ["linear", "cosine", "cosine_with_restarts", "polynomial", "constant", "constant_with_warmup"]:
            logger.error("[PARAMS] scheduler must be one of: linear, cosine, cosine_with_restarts, polynomial, constant, constant_with_warmup")
        ...
        if self.optimizer not in ["adamw_hf", "adamw_torch", "adamw_torch_fused", "adamw_apex_fused", "adafactor", "adamw_anyprecision", "sgd", "adagrad"]:
            logger.error("[PARAMS] optimizer must be one of: adamw_hf, adamw_torch, adamw_torch_fused, adamw_apex_fused, adafactor, adamw_anyprecision, sgd, adagrad")
        ...
        logger.info("[PARAMS] Miscellaneous parameters are valid.")
