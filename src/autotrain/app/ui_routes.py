...
@ui_router.post("/start_life_app_training", response_class=JSONResponse)
async def start_life_app_training(
    request: Request,
    project_name: str = Form(...),
    task: str = Form(...),
    base_model: str = Form(...),
    ...
    """
    Start training with LiFE App dataset
    """
    try:
        # Parse parameters
        ...
        config = {
            ...
            "username": autotrain_user,
            "token": token,
            "life_app_project": life_app_project,
            "life_app_script": life_app_script,
            "life_app_dataset_name": life_app_dataset_file
        }
        
        # Import and initialize integration
        from autotrain.trainers.automatic_speech_recognition.life_app.integration import LifeAppASRIntegration
        
        # Validate config
        if not LifeAppASRIntegration.validate_config(config):
            raise ValueError("Invalid training configuration")
            
        # Start training
        integration = LifeAppASRIntegration(config)
        integration.start_training()
